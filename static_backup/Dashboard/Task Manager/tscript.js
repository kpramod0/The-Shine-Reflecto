document.addEventListener('DOMContentLoaded', () => {
  ...
  if (menuBtn) {
    ...
    document.addEventListener('click', () => { if (!menuDrop.hidden) menuDrop.hidden = true; });
  }

  /* Footer active */
  const p = location.pathname.toLowerCase();
  document.querySelectorAll('.bottom-nav .nav-link').forEach(a => a.classList.remove('active'));
  if (p.includes('/member/')) document.querySelector('[data-key="member"]')?.classList.add('active');
  else if (p.includes('/management/')) document.querySelector('[data-key="management"]')?.classList.add('active');
  else if (p.includes('/dashboard/')) document.querySelector('[data-key="dashboard"]')?.classList.add('active');
  else document.querySelector('[data-key="home"]')?.classList.add('active');

  /* Demo data */
  const DATA = [
    { title:'Quarterly report', client:'Client A', assigned:'2025-01-30', due:'2025-09-21', completed:'2025-09-21', status:'Completed', priority:'High', review:'Good', notes:'Submitted on time' },
    { title:'Update project plan', client:'Client B', assigned:'2025-03-13', due:'2025-04-10', completed:'2025-04-13', status:'Completed', priority:'Medium', review:'Average', notes:'Minor delay due to scope change' },
    { title:'Data entry audit', client:'Client C', assigned:'2025-02-03', due:'2025-03-10', completed:'2025-03-08', status:'Completed', priority:'Low', review:'Good', notes:'All clean' },
    { title:'Bug triage', client:'Client A', assigned:'2025-04-22', due:'2025-05-02', completed:'2025-05-03', status:'Completed', priority:'High', review:'Good', notes:'Handled 17 issues' },
    { title:'Integration tests', client:'Client D', assigned:'2025-02-24', due:'2025-03-31', completed:'2025-03-31', status:'Completed', priority:'High', review:'Good', notes:'Full pass' },
    { title:'Policy review', client:'Client E', assigned:'2025-12-27', due:'2026-01-20', completed:'', status:'In Progress', priority:'Medium', review:'—', notes:'Drafting updates' },
    { title:'Risk register', client:'Client E', assigned:'2025-11-04', due:'2025-11-28', completed:'', status:'On Hold', priority:'Low', review:'—', notes:'Waiting stakeholder inputs' },
    { title:'Legacy cleanup', client:'Client C', assigned:'2025-08-11', due:'2025-09-01', completed:'2025-09-02', status:'Cancelled', priority:'Low', review:'Poor', notes:'Scope cancelled' },
  ];
  ...
  /* Collapsible – one arrow controls all */
  ...
  document.getElementById('btnPdf').addEventListener('click', ()=>{
    ...
    showToast('Use your browser’s dialog to save PDF');
  });
  ...
  function renderPerformance(rows){
    ...
    const pending = Math.max(0, total - completed); // anything not finished

    // Throughout = total work done (completed tasks)
    ...
    document.getElementById('statAvgCycle').textContent   = avgCycle;
